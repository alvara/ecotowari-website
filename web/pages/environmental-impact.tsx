...
// view all blog posts
const EnvironmentalImpact = ({posts}) => {
  return (
    <div>
        ...
        ({_id, title = '', slug, publishedAt = '', mainImage, categories}) =>
            ...
            <div  key={`${_id}`} >
              <InfoCard 
                className='postCard'
                mainImage={mainImage}
                href={"/EnvironmentalImpact/[slug]"}
                as={`/EnvironmentalImpact/${slug.current}`} 
                title={title} 
                tags={categories}
...
EnvironmentalImpact.getLayout = function getLayout(page: ReactElement) {
  return (
    <MainLayout>
      {page}
    </MainLayout>
